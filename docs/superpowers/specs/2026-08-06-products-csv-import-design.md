...
> **240 rows** · 12 new · 228 existing products updated · 0 errors
...
Deliberately "existing products updated", not "price updates": an upsert replaces Item Name and Item Code as well, so an old export re-imported to correct prices would silently revert every product renamed since. Naming only prices would understate what the owner is agreeing to.
